...
- (void)setupPublisher
{
	...
	[_publisher.view setFrame:
	 CGRectMake(10, self.view.frame.size.height - (100 + 110), 84, 110)];
...
{
    NSLog(@"session streamCreate: %@", stream);
...
{
    NSLog(@"client session DidConnect sucessful: %@", session);
    // now publish
	OTError *error;
...
{
    NSLog(@"client session DidConnect sucessful: %@", connection);
}
...
// 	[self createSubscriber:stream];
    NSLog(@"publisher streamCreate sucess.....");
}
...
{
	NSLog(@"sessionDidFail Error = %@",error);
	[self showAlert:
