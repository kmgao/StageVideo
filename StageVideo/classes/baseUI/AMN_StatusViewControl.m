...
@interface AMN_StatusViewControl()
{
    UILabel  *lstbarItem;
    UILabel  *cstbarItem;
    UILabel  *rstbarItem;
...
-(void)setTitle:(NSString*)title option:(int)dirOption{
    if(dirOption < 0 || dirOption > 2) return;
    if(dirOption == 0){
        if (lstbarItem == nil) {
            lstbarItem = [[UILabel alloc] init];
            [lstbarItem setBackgroundColor:[UIColor clearColor]];
            [lstbarItem setTextColor:[UIColor blackColor]];
            [lstbarItem setFont:[UIFont systemFontOfSize:12]];
            [lstbarItem setTextAlignment:dirOption];
            [lstbarItem setFrame:CGRectMake(8, 0, IPHONE_SCREEN_WIDTH-16, 20)];
            [self addSubview:lstbarItem];
        }
        [lstbarItem setText:title];
    }
    else if(dirOption == 1){
        if (cstbarItem == nil) {
            cstbarItem = [[UILabel alloc] init];
            [cstbarItem setBackgroundColor:[UIColor clearColor]];
            [cstbarItem setTextColor:[UIColor blackColor]];
            [cstbarItem setFont:[UIFont systemFontOfSize:12]];
            [cstbarItem setTextAlignment:dirOption];
            [cstbarItem setFrame:CGRectMake(8, 0, IPHONE_SCREEN_WIDTH-16, 20)];
            [self addSubview:cstbarItem];
        }
        [cstbarItem setText:title];
    }
    else if(dirOption == 2){
        if (rstbarItem == nil) {
            rstbarItem = [[UILabel alloc] init];
            [rstbarItem setBackgroundColor:[UIColor clearColor]];
            [rstbarItem setTextColor:[UIColor blackColor]];
            [rstbarItem setFont:[UIFont systemFontOfSize:12]];
            [rstbarItem setTextAlignment:dirOption];
            [rstbarItem setFrame:CGRectMake(8, 0, IPHONE_SCREEN_WIDTH-16, 20)];
            [self addSubview:rstbarItem];
        }
        [rstbarItem setText:title];
    }
}
